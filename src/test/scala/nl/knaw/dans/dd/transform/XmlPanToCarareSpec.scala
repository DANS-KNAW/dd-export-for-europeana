...
class XmlPanToCarareSpec extends TestSupportFixture with BeforeAndAfterEach {
  ...
  it should "produce a Carare XML-file with a correct digitalResource contents" in {
    ...
    (digitalResource \ "link").text  shouldBe "https://dar.dans.knaw.nl/api/access/datafile/1"
    (digitalResource \ "object").text  shouldBe "https://dar.dans.knaw.nl/api/access/datafile/1"
